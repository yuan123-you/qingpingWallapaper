  ...
  setup(__props) {
    ...
    async function loadHotSearch() {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/search/search.vue:125", "加载热门搜索失败:", error);
      }
    }
    ...
    async function loadSearchResults(refresh = false) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/search/search.vue:219", "搜索失败:", error);
      } finally {
        loading.value = false;
      }
    }
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        a: common_vendor.o([($event) => searchKeyword.value = $event.detail.value, handleInput]),

...
function isScopedSlotVFor({ source }) {
    if (!(0, uni_cli_shared_1.isCompoundExpressionNode)(source)) {
        return false;
    }
    const first = source.children[0];
    return ((0, uni_cli_shared_1.isSimpleExpressionNode)(first) &&
...
function findRootNode(root, context) {
    const children = root.children.filter((node) => (0, uni_cli_shared_1.isElementNode)(node) && node.tag !== 'template');
...
function createTransformContext(rootNode, { isX = false, root = '', filename = '', isTS = false, inline = false, hashId = null, scopeId = null, filters = [], bindingCssVars = [], bindingMetadata = shared_1.EMPTY_OBJ, cacheHandlers = false, prefixIdentifiers = false, skipTransformIdentifier = false, renderDataSpread = false, nodeTransforms = [], directiveTransforms = {}, miniProgram = {
    ...
    const context = {
        ...
        addIdentifiers(exp) {
            ...
            else if ((0, uni_cli_shared_1.isSimpleExpressionNode)(exp)) {
                addId(exp.content);
            }
        },
    ...
    return (node, context) => {
        if ((0, uni_cli_shared_1.isElementNode)(node)) {
            ...
            for (let i = 0; i < props.length; i++) {
                const prop = props[i];
                if ((0, uni_cli_shared_1.isDirectiveNode)(prop) && matches(prop.name)) {

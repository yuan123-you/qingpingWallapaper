...
function initPluginVueOptions(options, UniVitePlugins, uniPluginOptions) {
    ...
    compilerOptions.dom2 = process.env.UNI_APP_X_DOM2 === 'true';
    // 默认就移除comments节点
    ...
    if (options.platform !== 'h5' && options.platform !== 'web') {
        compilerOptions.nodeTransforms.push(...(0, uni_cli_shared_1.getBaseNodeTransforms)(options.base, process.env.UNI_VUE_VAPOR === 'true' ||
            process.env.UNI_APP_X_DOM2 === 'true'
            ? (0, uni_cli_shared_1.createResolveStaticAsset)(options.inputDir)
            : undefined));
    }
    if (nodeTransforms) {
    ...
    if (isX) {
        ...
        if (process.env.UNI_APP_X_DOM2 === 'true') {
            ...
            vueOptions.template.compilerOptions.extraOptions = (descriptor) => {
                const filename = (0, uni_cli_shared_1.normalizePath)(descriptor.filename.split('?')[0]);
                const relativeFilename = (0, uni_cli_shared_1.normalizePath)(path_1.default.relative(process.env.UNI_INPUT_DIR, filename));
                return {
                    root: (0, uni_cli_shared_1.normalizePath)(process.env.UNI_INPUT_DIR),
                    platform: process.env.UNI_UTS_PLATFORM,
                    componentType: (0, uni_cli_shared_1.isUniPageFile)(filename) ? 'page' : 'component',
                    relativeFilename,
                    helper: (0, uni_cli_shared_1.requireUniHelpers)(),
                    ...
                    onVueTemplateCompileLog(type, error) {
                        return (0, uni_cli_shared_1.onVueTemplateCompileLog)(type, error, descriptor.source, relativeFilename);
                    },
                };
            };
        }

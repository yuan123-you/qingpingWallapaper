...
async function transformMain(code, filename, options, pluginContext // 该 transformMain 方法被vuejs-core使用，编译框架内置组件了，此时不会传入pluginContext
    ...
    let templatePreambleMap = undefined;
    let templateCode = '';
    ...
    if (options.componentType !== 'app') {
        ...
        if (!isInline) {
            const { code, map, preamble, preambleMap } = (0, normalScript_1.processTemplate)(descriptor, {
                relativeFilename,
    ...
    if (options.sourceMap) {
        ...
        if (templateMap && (scriptMap || scriptCode)) {
            ...
            if (!scriptMap && scriptCode) {
                gen = new gen_mapping_1.GenMapping({
                    file: descriptor.relativeFilename,
                    sourceRoot: '',
                });
                (0, gen_mapping_1.setSourceContent)(gen, descriptor.relativeFilename, descriptor.source);
            }
            else {
                gen = (0, gen_mapping_1.fromMap)(
    ...
    if (templatePreambleCode && templatePreambleMap && pluginContext) {
        ...
        await (0, utils_1.parseImports)(templatePreambleCode, createTryResolve(filename, pluginContext.resolve, templatePreambleMap, 
        // 仅需要再解析script中的import，template上边已经加入了
        (source) => source.includes('/.uvue/') || source.includes('/.tsc/'), false // template中的资源路径解析，不支持external
        ));
    }
...
function createTryResolve(importer, resolve, resolvedMap, ignore, external = true) {
